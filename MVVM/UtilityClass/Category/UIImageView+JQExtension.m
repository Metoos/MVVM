...
- (void)jq_setImageWithURL:(NSString *)url;
{
    ...
    [self jq_setImageWithURL:url placeholderImage:placeholderImg options:SDWebImageRetryFailed];
}
...
{
    [[SDImageCache sharedImageCache] removeImageForKey:key fromDisk:YES withCompletion:NULL];
}
...
{
    [self jq_setImageWithURL:url placeholderImage:placeholderImage width:width completed:NULL];
}


// 返回固定宽 高按图片宽高比自动缩放 设置尺寸的网络图片
- (void)jq_setImageWithURL:(NSString *)url placeholderImage:(UIImage *)placeholderImage width:(CGFloat)width completed:(void(^)(UIImage *image))completed
{
    NSURL *nsUrl = [NSURL URLWithString:url];
    ...
    [self sd_setImageWithURL:nsUrl placeholderImage:placeholderImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ...
        if (image) {
            ...
            UIImage *images = [UIImage resizedImage:image toSize:size];
            weakSelf.image = images;
