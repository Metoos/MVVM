...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.advertScrollViewStyle == SGAdvertScrollViewStyleMore) {
        ...
        if ([imagePath hasPrefix:@"http"]) {
            [cell.signImageView jq_setImageWithURL:imagePath placeholderImage:placeholderImg width:30.0f];
        } else {
            if(imagePath)
            {
        ...
        cell.titleLabel.text = [[NSString alloc]initWithFormat:@"  %@",self.titleArr[indexPath.item]];
