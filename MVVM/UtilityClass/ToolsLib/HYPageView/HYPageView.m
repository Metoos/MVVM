...
#define  NAVIGATIONBAR_STATUSBAR_HEIGHT [[UIApplication sharedApplication] statusBarFrame].size.height+44.0f
...
- (instancetype)initWithFrame:(CGRect)frame withTitles:(NSArray *)titles withViewControllers:(NSArray *)controllers withParameters:(NSArray *)parameters{
    ...
    if (self) {
        ...
        _topSpace              = STATUSBAR_HEIGHT;
        _isAverage             = YES;
        _isShowTopTabBottomLine= YES;
        _isTranslucent         = YES;
        _isAnimated            = NO;
        self.isAdapteNavigationBar = YES;
...
- (void)setIsAdapteNavigationBar:(BOOL)isAdapteNavigationBar
{
    _isAdapteNavigationBar = isAdapteNavigationBar;
    
    if (@available(iOS 11.0, *)) {
        self.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
        if (isAdapteNavigationBar) {
            self.contentInset = UIEdgeInsetsMake(NAVIGATIONBAR_STATUSBAR_HEIGHT, 0, 0, 0);
        }else
        {
...
- (UIView *)topTabView{
    if (!_topTabView){
        ...
        if (_viewController.navigationController && !_viewController.navigationController.navigationBar.hidden && !_viewController.navigationController.navigationBarHidden && !_isAdapteNavigationBar) {
            y = - (NAVIGATIONBAR_STATUSBAR_HEIGHT);
        }
        ...
        for (NSInteger i=0; i<_viewControllers.count; i++) {
            if (page == i) {
                ...
                if (![className isEqualToString:@"HYPAGEVIEW_Controller"]) {
                    ...
                    if ([className isEqualToString:@"PublicRecordViewController"]) {
                        if (_parameters && _parameters.count > i && _parameters[i]) {
                            viewController =  [[class alloc]initWithViewModel:_parameters[i]];
                        }
                        
                    }else if ([NSStringFromClass(class_getSuperclass(class)) isEqualToString:NSStringFromClass(JQTableViewController.class)]) {
                        if (_parameters && _parameters.count > i && _parameters[i]) {
                            viewController =  [[class alloc]initWithViewModel:_parameters[i]];
                        }
                ...
                if (_viewController.navigationController && !_viewController.navigationController.navigationBar.hidden && !_viewController.navigationController.navigationBarHidden && _isAdapteNavigationBar) {
                    offset += NAVIGATIONBAR_STATUSBAR_HEIGHT;
                }
